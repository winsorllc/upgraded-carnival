...
function printHeader() {
  if (quiet) return;
  console.log(colors.cyan + 'ΓòÉ'.repeat(64) + colors.reset);
...
if (fix) {
  ...
  if (result.fixed) {
    console.log(colors.green + `Γ£ô Fixed formatting: ${result.file}` + colors.reset);
  } else {
    console.log(colors.blue + `ΓÇó Already formatted: ${result.file}` + colors.reset);
  }
  process.exit(0);
}
...
for (const file of files) {
  ...
  if (result.valid) {
    validCount++;
    if (!quiet) {
      console.log(`${colors.green}Γ£ô${colors.reset} ${result.file}`);
      console.log(`  Valid JSON - ${result.size}, ${result.lines} lines`);
      console.log();
    }
  } else {
    errorCount++;
    console.error(`${colors.red}Γ£ù${colors.reset} ${result.file}`);
...
if (fixAll) {
  ...
  for (const file of files) {
    ...
    if (fixResult.fixed) {
      console.log(`${colors.green}Γ£ô${colors.reset} Fixed: ${fixResult.file}`);
    } else if (fixResult.error) {
      console.log(`${colors.red}Γ£ù${colors.reset} Cannot fix: ${fixResult.file} - ${fixResult.error}`);
    }
  }
  console.log();
}
...
// Summary
if (!quiet) {
  console.log(colors.cyan + 'ΓöÇ'.repeat(64) + colors.reset);
